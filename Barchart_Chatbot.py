...
st.sidebar.title("Welcome to Barchart!")
st.sidebar.info("""
This Chatbot will solve your annual report-related questions for the Fortune 500 companies! 
- Data covers annual reports from **2012 to 2023**.
- Focuses on financial ratios and other **text-based information** from the reports.
- Questions should specify the company name, the year of relevant information, ratio and as much detail as possible.
...
if "messages" not in st.session_state:
    st.session_state.messages = []  # Initialize as an empty list if not already present

# Ensure session ID is initialized
if "session_id" not in st.session_state:
    st.session_state.session_id = "session_1"

# Add a "Clear chat" button to reset the chat history
...
if clear_chat:
    st.cache_data.clear()  # Clear the cached chat history
    st.session_state.messages = []  # Reset the in-memory chat history

# Apply custom CSS to style the "Clear Chat" button
st.markdown(
    """
    <style>
    div.stButton > button {
        ...
        padding: 5px 15px;        /* Optional: Adjust padding */
...
# messages_html = "\n".join(
#     f"<div class='{('user-message' if msg['role'] == 'user' else 'assistant-message')}'>{format_text_for_consistency(msg['content'])}</div>"
#     for msg in st.session_state.messages
# )
# st.markdown(f"<div class='chat-container message-container'>{messages_html}</div>", unsafe_allow_html=True)
...
if prompt := st.chat_input("Enter your Financial Queries here: "):
